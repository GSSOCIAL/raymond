...
<table id="dashletPanel" cellpadding='0' cellspacing='0' width='100%' border='0' class='list view default'>
	...
	{foreach name=rowIteration from=$data key=id item=rowData}
		...
		<tr height='20' class='{$_rowColor}S1'>
			{if $prerow}
			<td width='1%' nowrap='nowrap' style="background-color: #FDFB57;">
			...
			{if !empty($quickViewLinks)}
			<td width='1%' class='{$_rowColor}S1' style="background-color:#FDFB57;" nowrap>
				{if $pageData.rowAccess[$id].edit}
					...
					<a title='{$editLinkString}'
					   href='index.php?action=EditView&module={$pageData.bean.moduleDir}&record={$rowData.ID}&offset={$pageData.offsets.current+$smarty.foreach.rowIteration.iteration}&stamp={$pageData.stamp}&return_module=Home&return_action=index'>{sugar_getimage name="edit_inline.png" attr='border="0" ' alt="$alt_edit"}</a>
				{/if}
				{if $pageData.access.view}
				<a title='{$viewLinkString}'
				   href='index.php?action=DetailView&module={$params.module|default:$pageData.bean.moduleDir}&record={$rowData[$params.parent_id]|default:$rowData.ID}&offset={$pageData.offsets.current+$smarty.foreach.rowIteration.iteration}&stamp={$pageData.stamp}&return_module=Home&return_action=index'
				   title="{sugar_translate label="LBL_VIEW_INLINE"}>{sugar_getimage name="view_inline.png" attr='border="0" ' alt="$alt_view"}</a>
				{/if}
			</td>
			{/if}
