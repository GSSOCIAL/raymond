{if $ReadOnly == "readonly"}
    {if !$fields.lic_type.hidden}
        {counter name="panelFieldCount" print=false}

        {if !empty($fields.lic_type.value) && ($fields.lic_type.value != '^^')}
            <input type="hidden" class="sugar_field" id="{$fields.lic_type.name}" value="{$fields.lic_type.value}">
            {multienum_to_array string=$fields.lic_type.value assign="vals"}
            {foreach from=$vals item=item}
                <li style="margin-left:10px;">{ $fields.lic_type.options.$item }</li>
            {/foreach}
        {/if}
    {/if}
{else}
    ...
    {if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
        <input type="hidden" id="{$fields.lic_type.name}_multiselect"
               name="{$fields.lic_type.name}_multiselect" value="true">
        {multienum_to_array string=$fields.lic_type.value default=$fields.lic_type.default assign="values"}
        <select id="{$fields.lic_type.name}"
                name="{$fields.lic_type.name}[]"
                multiple="true" size='6' style="width:150" title='' tabindex=""
        >
            {html_options options=$fields.lic_type.options selected=$values}
        </select>
    {else}
        {assign var="field_options" value=$fields.lic_type.options }
        {capture name="field_val"}{$fields.lic_type.value}{/capture}
        {assign var="field_val" value=$smarty.capture.field_val}
        {capture name="ac_key"}{$fields.lic_type.name}{/capture}
        {assign var="ac_key" value=$smarty.capture.ac_key}
        <input type="hidden" id="{$fields.lic_type.name}_multiselect"
               name="{$fields.lic_type.name}_multiselect" value="true">
        {multienum_to_array string=$fields.lic_type.value default=$fields.lic_type.default assign="values"}
        <select style='display:none' id="{$fields.lic_type.name}"
                name="{$fields.lic_type.name}[]"
                multiple="true" size='6' style="width:150" title='' tabindex=""
        >
            {html_options options=$fields.lic_type.options selected=$values}
        ...
        <input
                id="{$fields.lic_type.name}-input"
                name="{$fields.lic_type.name}-input"
                size="60"
                type="text" style="vertical-align: top;">
        <span class="id-ff multiple">
<button type="button">
<img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.lic_type.name}-image">
</button>
<button type="button"
        id="btn-clear-{$fields.lic_type.name}-input"
        title="Clear"
        onclick="SUGAR.clearRelateField(this.form, '{$fields.lic_type.name}-input', '{$fields.lic_type.name};');SUGAR.AutoComplete.{$ac_key}.inputNode.updateHidden()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
    {literal}
        <script>
            ...
            YUI().use('datasource', 'datasource-jsonschema', function (Y) {
                SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.ds = new Y.DataSource.Function({
                    source: function (request) {
                        var selectElem = document.getElementById("{/literal}{$fields.lic_type.name}{literal}");
                        var ret = [];
            ...
            YUI().use("autocomplete", "autocomplete-filters", "autocomplete-highlighters","node-event-simulate", function (Y) {
                ...
                SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.lic_type.name}-input');
                SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.lic_type.name}-image');
                SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.lic_type.name}');
                ...
                function updateTextOnInterval(){
                    if (document.activeElement != document.getElementById("{/literal}{$fields.lic_type.name}-input{literal}"))
