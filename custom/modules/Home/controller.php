...
class CustomHomeController extends SugarController{
    /**
     * Performs document download
     */
    function action_beanServer(){
        ...
        if(!empty($allowed_methods[$_REQUEST["method"]])&&$allowed_methods[$_REQUEST["method"]]===true){
            ...
            switch($_REQUEST["method"]){
                case "export":
                    ...
                    $BeanExport = new BeanExport($_REQUEST["mod"],$_REQUEST["record"],array(
                        "internal"=>array_key_exists("internals",$_REQUEST)&&$_REQUEST["internals"]=='true'
                    ));
