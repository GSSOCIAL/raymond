...
class SugarWidgetSubPanelTopComposeEmailButtonCase extends SugarWidgetSubPanelTopButton
{
	var $form_value = '';
	...
	function display($defines, $additionalFormFields = NULL, $nonbutton = false)
	{
		...
		if($client != 'sugar') {
			...
			if (isset($bean->emailAddress)){
				...
				$button = "<input class='button' casescompose='yes' type='button'  value='$value'  id='". $this->getWidgetId() . "'  name='".preg_replace('[ ]', '', $value)."'   title='$title' onclick=\"location.href='mailto:$to_addrs';return false;\" />";
			}
			else{
				$button = "<input class='button' casescompose='yes' type='button'  value='$value'  id='". $this->getWidgetId() ."'  name='".preg_replace('[ ]', '', $value)."'  title='$title' onclick=\"location.href='mailto:';return false;\" />";
			}
		} else {
            ...
            $button = "<input title='$title' casescompose='yes'  id='". $this->getWidgetId()."'  onclick='SUGAR.quickCompose.init($j_quickComposeOptions);' class='button' type='submit' name='".preg_replace('[ ]', '', $value)."_button' value='$value' />";
		}
		return $button;
	}
}
