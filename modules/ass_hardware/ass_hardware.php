 ...
 */
require_once('modules/ass_hardware/ass_hardware_sugar.php');
class ass_hardware extends ass_hardware_sugar {
	...
	public function create_new_list_query($order_by, $where, $filter = array(), $params = array(), $show_deleted = 0, $join_type = '', $return_array = false, $parentbean = null, $singleSelect = false, $ifListForExport = false) {
		...
		if ($_REQUEST['action'] == 'quicksearchQuery') {
			$ret_array['from'] .= "
								LEFT JOIN
									`ass_hardware_accounts_c`
									ON `ass_hardware_accounts_c`.`ass_hardware_accountsass_hardware_idb` = `ass_hardware`.`id`
									AND `ass_hardware_accounts_c`.`deleted` = 0
			";
		}
		if ($return_array) {
			return $ret_array;
