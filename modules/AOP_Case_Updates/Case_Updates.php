...
function display_updates($focus)
{
    ...
    if($hw && $hw->id){
        ...
        //Check for buttons
        if(!empty($hw->pass_w)){
            $sugar_smarty->assign('COPY_WEB_PASSWORD_BUTTON', '<button class="copy_button button" data-clipboard-text="'.$hw->pass_w.'">Copy WEB Password</button>');
        }
        if(!empty($hw->pass_r)){
            $sugar_smarty->assign('COPY_ROOT_PASSWORD_BUTTON', '<button class="copy_button button" data-clipboard-text="'.$hw->pass_r.'">Copy ROOT Password</button>');
        }
    }
